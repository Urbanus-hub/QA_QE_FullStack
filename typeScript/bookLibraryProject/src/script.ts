document.addEventListener("DOMContentLoaded", function () {
  ...
  const cartTotalItems = document.querySelector(".cart-total span:last-child") as HTMLElement;
      ...
      books.forEach((result: Book) => {
        ...
        buyBook.addEventListener('click', function(e) {
          ...
          if (target && target.parentNode) {
            const idElement = (target.parentNode as HTMLElement).querySelector('.id-book') as HTMLElement;
            if (idElement) {
  ...
  function showBookModal(book: Book) {
    const modal = document.getElementById("book-modal") as HTMLElement;
    if (modal) {
      modal.style.display = "flex";
      
      // Populate modal content
      const modalImg = modal.querySelector(".modal-book-image img") as HTMLImageElement;
      if (modalImg) {
      ...
      const addToCartBtn = modal.querySelector(".primary-btn") as HTMLElement;
      if (addToCartBtn) {
        addToCartBtn.textContent = `Add to Cart • $${book.price.toFixed(2)}`;
        
        // Remove previous event listeners
        const newAddToCartBtn = addToCartBtn.cloneNode(true) as HTMLElement;
