document.addEventListener("DOMContentLoaded", function (){
  ...
  if (addBookForm) {
    addBookForm.addEventListener('submit', async (e) => {
      ...
      const genre = (document.getElementById('add-genre') as HTMLSelectElement).value;
      const year = (document.getElementById('add-year') as HTMLInputElement).value;
      const pages = (document.getElementById('add-pages') as HTMLInputElement).value;
      const publisher = (document.getElementById('add-publisher') as HTMLInputElement).value;
      const description = (document.getElementById('add-description') as HTMLTextAreaElement).value;
      const image = (document.getElementById('add-image') as HTMLInputElement).value;
      
      // Use the price field if it exists, otherwise generate a random price
      let price;
      const priceInput = document.getElementById('price') as HTMLInputElement;
      if (priceInput && priceInput.value) {
        price = parseFloat(priceInput.value);
      } else {
        // Generate random price between $9.99 and $29.99
        price = (Math.random() * 20 + 9.99).toFixed(2);
      }
      
      // Construct book data object exactly matching API requirements
      const bookData = {
        title,
        author,
        genre,
