...
const popularPost= activeUsers.map((user)=>{
    ...
    const flattenedPosts = popularPost.reduce((acc, currentArray) => {
        return [...acc, ...currentArray]
    }, [])
    
    // Calculate average
    const averageLikes = flattenedPosts.reduce((acc, post) => acc + post.likes, 0) / flattenedPosts.length
